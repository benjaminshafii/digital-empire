...
It is based on patterns in:

- `.opencode/agent/draupnir.md`
- `.opencode/skill/home-assistant/SKILL.md`
- `.opencode/skill/qbittorrent/SKILL.md`
- `.opencode/skill/telegram/SKILL.md`
...
├── .skill.env.example           # Tracked. Declares required env vars.
├── .skill.env                   # Gitignored. Actual credentials.
...
- If a skill does not need credentials, you can omit `.skill.env*` and `load-env.ts`.
...
If it’s sensitive, it goes in the local overlay (`.skill.env`, `*.json`, OS keychain, Bitwarden).
...
### Primary: per-skill `.skill.env`

- Store credentials in `.opencode/skill/<skill-name>/.skill.env` (gitignored).
- Document required variables in `.skill.env.example`.
...
- **Local `.skill.env`**: fastest and simplest.
...
- If `.skill.env` missing, print clear “Blocked” instructions (don’t guess secrets).
...
loadDotenv({ path: resolve(import.meta.dir, ".skill.env") });
...
- Don’t auto-create `.skill.env` with secrets; only create `.skill.env` from `.skill.env.example` with blanks.
