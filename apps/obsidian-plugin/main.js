...
function transformWikiLinks(content, basePath = "/blog") {
  ...
  const codeBlocks = [];
  const contentWithPlaceholders = content.replace(codeBlockRegex, (match2) => {
  ...
  const transformed = contentWithPlaceholders.replace(
    ...
    (_match, link, displayText) => {
      ...
      const text2 = displayText || link;
      return `[${text2}](${basePath}/${slug})`;
    }
  );
...
function extractFrontmatter(markdown) {
  ...
  const content = markdown.slice(match2[0].length);
  const frontmatter = {};
...
var SyncService = class {
  ...
  extractEmbeddedImageLinkpaths(markdown) {
    const codeBlockRegex = /```[\s\S]*?```|`[^`]+`/g;
    const codeBlocks = [];
    const contentWithPlaceholders = markdown.replace(codeBlockRegex, (match3) => {
      codeBlocks.push(match3);
      return `__CODE_BLOCK_${codeBlocks.length - 1}__`;
    });
    const embedRegex = /!\[\[([^\]]+)\]\]/g;
    const imageExtensions = /\.(png|jpg|jpeg|gif|svg|webp|bmp)$/i;
    const linkpaths = [];
    let match2;
    while ((match2 = embedRegex.exec(contentWithPlaceholders)) !== null) {
      const rawLinkpath = match2[1].split("|")[0].trim();
...
var WebsiteSyncSettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian2.Setting(containerEl).setName("Source folder").setDesc("Folder containing notes to sync (notes must have publish: true in frontmatter)").addText(
      (text2) => text2.setPlaceholder("publish").setValue(this.plugin.settings.sourceFolder).onChange(async (value) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("GitHub token").setDesc("Personal access token with repo scope").addText(
      (text2) => text2.setPlaceholder("ghp_xxxx...").setValue(this.plugin.settings.githubToken).onChange(async (value) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("GitHub owner").setDesc("GitHub username or organization").addText(
      (text2) => text2.setPlaceholder("username").setValue(this.plugin.settings.githubOwner).onChange(async (value) => {
        this.plugin.settings.githubOwner = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian2.Setting(containerEl).setName("GitHub repository").setDesc("Repository name").addText(
      (text2) => text2.setPlaceholder("my-website").setValue(this.plugin.settings.githubRepo).onChange(async (value) => {
        this.plugin.settings.githubRepo = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian2.Setting(containerEl).setName("Branch").setDesc("Git branch to sync to").addText(
      (text2) => text2.setPlaceholder("main").setValue(this.plugin.settings.branch).onChange(async (value) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Target path").setDesc("Path in repository where posts are stored").addText(
      (text2) => text2.setPlaceholder("apps/blog/src/content/blog").setValue(this.plugin.settings.targetPath).onChange(async (value) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Collaboration server URL").setDesc("WebSocket URL for the collaboration server").addText(
      (text2) => text2.setPlaceholder("ws://127.0.0.1:1234").setValue(this.plugin.settings.collabServerUrl).onChange(async (value) => {
        this.plugin.settings.collabServerUrl = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian2.Setting(containerEl).setName("Collaboration autosave").setDesc("Automatically write collaborative edits back to the vault").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.collabAutosave).onChange(async (value) => {
        this.plugin.settings.collabAutosave = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian2.Setting(containerEl).setName("Autosave debounce (ms)").setDesc("How long to wait before writing changes back to disk").addText(
      (text2) => text2.setPlaceholder("1500").setValue(String(this.plugin.settings.collabAutosaveDebounceMs)).onChange(async (value) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Export folder").setDesc("Vault folder to store exported collaborative snapshots").addText(
      (text2) => text2.setPlaceholder("collab").setValue(this.plugin.settings.collabExportFolder).onChange(async (value) => {
...
// ../../node_modules/.pnpm/y-codemirror.next@0.3.5_@codemirror+state@6.5.3_@codemirror+view@6.39.8_yjs@13.6.29/node_modules/y-codemirror.next/src/index.js
var cmView4 = __toESM(require("@codemirror/view"), 1);
var cmState4 = __toESM(require("@codemirror/state"), 1);

// ../../node_modules/.pnpm/y-codemirror.next@0.3.5_@codemirror+state@6.5.3_@codemirror+view@6.39.8_yjs@13.6.29/node_modules/y-codemirror.next/src/y-range.js
...
// ../../node_modules/.pnpm/y-codemirror.next@0.3.5_@codemirror+state@6.5.3_@codemirror+view@6.39.8_yjs@13.6.29/node_modules/y-codemirror.next/src/y-sync.js
...
// ../../node_modules/.pnpm/y-codemirror.next@0.3.5_@codemirror+state@6.5.3_@codemirror+view@6.39.8_yjs@13.6.29/node_modules/y-codemirror.next/src/y-remote-selections.js
...
// ../../node_modules/.pnpm/y-codemirror.next@0.3.5_@codemirror+state@6.5.3_@codemirror+view@6.39.8_yjs@13.6.29/node_modules/y-codemirror.next/src/y-undomanager.js
...
// ../../node_modules/.pnpm/y-websocket@3.0.0_yjs@13.6.29/node_modules/y-websocket/src/y-websocket.js
var messageSync = 0;
...
function createRoomNameFromPath(filePath) {
...
var CollabSession = class {
  constructor(serverUrl, roomName) {
    ...
    this.undoManager = new UndoManager(this.text);
  ...
  ensureInitializedFromLocal(localText) {
...
// src/ui/collab-view.ts
var COLLAB_VIEW_TYPE = "collab-note-view";
var CollabNoteView = class extends import_obsidian4.ItemView {
  constructor(leaf, plugin) {
    super(leaf);
    this.filePath = null;
  ...
  teardown() {
    ...
    this.editorView = null;
    (_b = this.session) == null ? void 0 : _b.destroy();
    this.session = null;
  }
  async openForCurrentState() {
    this.teardown();
    const container = this.containerEl.children[1];
    container.empty();
    ...
    if (!serverUrl) {
      new import_obsidian4.Notice("Set Collaboration server URL in settings");
    ...
    this.session.provider.on("sync", (isSynced) => {
      var _a;
      if (isSynced) {
        (_a = this.session) == null ? void 0 : _a.ensureInitializedFromLocal(localText);
      }
    });
    ...
    const state = import_state.EditorState.create({
      ...
      extensions: [
        ...
        import_view.keymap.of([
          ...
          { key: "Mod-z", run: import_commands.undo },
          { key: "Mod-Shift-z", run: import_commands.redo }
        ]),
        yCollab(this.session.text, this.session.provider.awareness, {
          undoManager: this.session.undoManager
        }),
        import_view.EditorView.updateListener.of(() => {
  ...
  scheduleSave(file) {
    ...
    this.saveTimer = window.setTimeout(async () => {
      if (!this.session) return;
      const text2 = this.session.text.toString();
      if (text2 === this.lastWrittenText) return;
      this.lastWrittenText = text2;
      try {
        await this.app.vault.modify(file, text2);
      } catch (error) {
        new import_obsidian4.Notice(`Failed to save: ${error instanceof Error ? error.message : "Unknown error"}`);
      }
    }, debounceMs);
  }
};
...
function formatTimestampForPath(date) {
  ...
  const min2 = pad2(date.getMinutes());
  const ss = pad2(date.getSeconds());
  return `${yyyy}${mm}${dd}-${hh}${min2}${ss}`;
}
function sanitizeBasenameForPath(name) {
  return name.trim().replace(/\s+/g, "-").replace(/[^a-zA-Z0-9._-]/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "").slice(0, 80);
}
function buildCollabSnapshotPath(notePath, exportFolder, now = /* @__PURE__ */ new Date()) {
  var _a;
  const filename = (_a = notePath.split("/").pop()) != null ? _a : "note.md";
...
// src/main.ts
var WebsiteSyncPlugin = class extends import_obsidian5.Plugin {
  constructor() {
    super(...arguments);
  ...
  async syncNotes() {
    ...
    if (!this.settings.githubToken) {
      new import_obsidian5.Notice("Please configure your GitHub settings first");
      return;
    }
    new import_obsidian5.Notice("Starting sync...");
    try {
      ...
      if (summary.warnings.length > 0) {
        for (const warning of summary.warnings) {
          new import_obsidian5.Notice(`Warning: ${warning}`, 5e3);
        }
      }
      if (summary.failed > 0) {
        const failedNotes = summary.results.filter((r) => !r.success);
        const errorMsg = failedNotes.map((r) => r.error).join(", ");
        new import_obsidian5.Notice(`Sync failed: ${errorMsg}`, 1e4);
      } else if (summary.synced > 0) {
        new import_obsidian5.Notice(`Synced ${summary.synced} notes`);
      } else {
        new import_obsidian5.Notice("No notes to sync");
      }
    } catch (error) {
      console.error("Sync error:", error);
      new import_obsidian5.Notice(`Sync failed: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }
  refreshSyncView() {
  ...
  async openCollabView() {
    const activeFile = this.app.workspace.getActiveFile();
    if (!activeFile) {
      new import_obsidian5.Notice("Please open a file first");
      return;
    }
    const { workspace } = this.app;
  ...
  async quickPublish(file) {
    ...
    if (targetFile.extension !== "md") {
      new import_obsidian5.Notice("Can only publish markdown files");
      return;
    }
    try {
...
`;
        ...
        new import_obsidian5.Notice("Added frontmatter");
      } else {
        const frontmatterRegex = /^---\n([\s\S]*?)\n---/;
...
---`);
      ...
      if (!targetFile.path.startsWith(sourceFolder + "/")) {
        ...
        if (this.app.vault.getAbstractFileByPath(newPath)) {
          new import_obsidian5.Notice(`File already exists at ${newPath}`);
          return;
        }
        await this.app.fileManager.renameFile(targetFile, newPath);
        new import_obsidian5.Notice(`Moved to ${newPath}`);
      }
      await this.syncNotes();
    } catch (error) {
      console.error("Quick publish error:", error);
      new import_obsidian5.Notice(`Quick publish failed: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }
  async exportCollabSnapshot() {
    var _a;
    const activeFile = this.app.workspace.getActiveFile();
    if (!activeFile) {
      new import_obsidian5.Notice("No file selected");
    ...
    if (activeFile.extension !== "md") {
      new import_obsidian5.Notice("Can only export markdown files");
      return;
    }
    const leaves = this.app.workspace.getLeavesOfType(COLLAB_VIEW_TYPE);
    const collabView = (_a = leaves[0]) == null ? void 0 : _a.view;
    if (!collabView || !(collabView instanceof CollabNoteView)) {
      new import_obsidian5.Notice("Please open the collaboration view for the active note");
      return;
    }
    const collabText = collabView.getCurrentText();
    if (!collabText) {
      new import_obsidian5.Notice("No collaborative content to export");
      return;
    }
    ...
    try {
      ...
      new import_obsidian5.Notice(`Exported snapshot to ${exportPath}`);
    } catch (error) {
      console.error("Export error:", error);
      new import_obsidian5.Notice(`Export failed: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }
};
