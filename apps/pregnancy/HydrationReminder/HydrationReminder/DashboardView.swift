            ...
            ScrollView {
                VStack(spacing: 20) {
                    ...
                    PregnancyFunFactCard(
                        currentWeek: pregnancyManager.isPregnancyDataSet ? pregnancyManager.pregnancyData?.currentWeek : nil
                    )
    ...
    private var recentActivityContent: some View {
        Group {
            if logsManager.getTodayLogs().isEmpty {
                // iOS 26 ContentUnavailableView for empty state with liquid glass styling
                ...
                // iOS 26 Best Practice: Dynamic list sizing without fixed heights
                VStack(spacing: 0) {
                    ForEach(Array(logsManager.getTodayLogs().prefix(5))) { log in
                        ...
                        // Subtle separator between rows (iOS 26 pattern)
                        if log.id != logsManager.getTodayLogs().prefix(5).last?.id {
                            Divider()
                                .padding(.leading, 58)
                        }
                    }
